...
import { missionSets } from "@/data/dailyMissions";
...
export default function DailyInspiration({ compact = false }) {
  ...
  const [viewedQuoteIds, setViewedQuoteIds] = useState([]);
  ...
  useEffect(() => {
    ...
    if (!studyWisdomQuotes || studyWisdomQuotes.length === 0) {
      return;
    }
  ...
  return (
    <div className="bg-gradient-to-br from-brand-500/10 via-transparent to-surface/50 border border-white/10 rounded-2xl p-5 space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Today's Missions */}
      {dailyMissions.content && dailyMissions.content.length > 0 && (
        <div className="flex items-start gap-4">
