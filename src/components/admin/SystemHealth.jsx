  ...
  return (
    <div className="border border-white/10 rounded-xl p-6">
      ...
      <div className="space-y-4">
        <div
          className={`flex items-center justify-between p-4 rounded-lg border ${getStatusColor(health.database.status)}`}
        >
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p
                className={`text-sm font-medium ${getLatencyColor(health.database.latency)}`}
              >
        ...
        <div
          className={`flex items-center justify-between p-4 rounded-lg border ${getStatusColor(health.api.status)}`}
        >
          <div className="flex items-center gap-3">
            <Server
              className={
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p
                className={`text-sm font-medium ${getLatencyColor(health.api.latency)}`}
              >
        ...
        <div
          className={`flex items-center justify-between p-4 rounded-lg border ${getStatusColor(health.storage.status)}`}
        >
          <div className="flex items-center gap-3">
            <HardDrive
              className={
                health.storage.status === "online"
                  ? "text-green-400"
                  : "text-red-400"
              }
              size={20}
