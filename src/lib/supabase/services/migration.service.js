...
export class MigrationService {
  ...
  /**
    ...
    */
  async migrateToSupabase(userId, data) {
    ...
    if (data.readingHistory && data.readingHistory.length > 0) {
      try {
        const historyToInsert = data.readingHistory.map((h) => ({
          user_id: userId,
    ...
    if (data.highlights && data.highlights.length > 0) {
      try {
        const highlightsToInsert = data.highlights.map((h) => ({
          user_id: userId,
          passage_id: h.passageId,
          text: h.text,
